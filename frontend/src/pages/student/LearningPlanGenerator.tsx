...
import { Sparkles, BrainCircuit, Library, Loader2, ArrowRight, ShieldAlert, Target, BookOpen, Layers, CheckCircle2, TrendingUp, NotebookPen, Lightbulb } from 'lucide-react';
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400">Personalized Learning Plan</p>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-8">
          ...
          <Card className="bg-purple-500/10 border-purple-500/20">
            <CardContent className="p-4">
              <p className="text-sm font-medium text-purple-400">AI Confidence Score</p>
              <p className="text-xl font-bold text-white">{plan.score}/100</p>
            </CardContent>
          </Card>
          <Card className="bg-green-500/10 border-green-500/20">
            <CardContent className="p-4">
              <p className="text-sm font-medium text-green-400">Evaluation Iterations</p>
              <p className="text-xl font-bold text-white">{plan.iteration_count}</p>
            </CardContent>
          </Card>
        </div>
          ...
          <Card className="border-yellow-500/30 bg-yellow-500/5">
            <CardHeader className="pb-2">
              <CardTitle className="text-yellow-400 flex items-center text-lg">
                <ShieldAlert className="w-5 h-5 mr-2" /> Note from AI Evaluator
              </CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <Card>
          <CardHeader className="pb-4 border-b border-white/5">
            <CardTitle className="text-xl flex items-center">
              <Layers className="w-6 h-6 mr-2 text-blue-400" /> Learning Roadmap & Modules
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-6 space-y-6">
            ...
            <div className="pt-4 flex justify-end">
              <Button onClick={() => navigate('/student/dashboard')} className="px-8">
                Save & Start Studying <ArrowRight className="w-4 h-4 ml-2" />
              </Button>
            </div>
          </CardContent>
        </Card>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-gray-400">Generate a personalized curriculum using RAG and multi-agent AI.</p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-semibold flex items-center text-gray-200">
                <Library className="w-5 h-5 mr-2 text-primary" /> RAG Filters
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Input label="Year" name="year" placeholder="3" value={formData.year} onChange={handleChange} required />
                <Input label="Semester" name="semester" placeholder="5" value={formData.semester} onChange={handleChange} required />
                <Input label="Regulation" name="regulation" placeholder="2023" value={formData.regulation} onChange={handleChange} required />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-semibold flex items-center text-gray-200">
                <BrainCircuit className="w-5 h-5 mr-2 text-purple-400" /> Learning Request
              </h3>
              <Input label="Subject" name="subject" placeholder="Web Technology" value={formData.subject} onChange={handleChange} required />
              <Input label="Topic" name="topic" placeholder="React Hooks" value={formData.topic} onChange={handleChange} required />
              <Input label="Learning Goal" name="learning_goal" placeholder="Understand useState, useEffect and custom hooks" value={formData.learning_goal} onChange={handleChange} required />
            </div>
